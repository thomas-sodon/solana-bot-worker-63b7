...
export async function formatAmmKeysToApi(connection: Connection, programId: string, findLookupTableAddress: boolean = false): Promise<ApiPoolInfo> {
  return {
    official: [],
    unOfficial: await formatAmmKeys(connection, programId, findLookupTableAddress)
  }
}